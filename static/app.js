...
async function loadOllamaUi() {
  const sel = document.getElementById('ollamaModelSelect');
  const hostEl = document.getElementById('ollamaHostDisplay');
  const statusEl = document.getElementById('ollamaCheckStatus');
  const defaultBtn = document.getElementById('ollamaDefaultBtn');
  if (!sel) return;
  try {
    const check = await api('/api/ai/ollama/check');
    if (hostEl) hostEl.textContent = check.ollama_host || '…';
    if (statusEl) {
      if (!check.reachable) {
        statusEl.textContent = `⚠ ${check.error || 'Ollama unreachable'}`;
        statusEl.style.color = 'var(--danger)';
      } else if (check.model_ok === false && check.model_error) {
        statusEl.textContent = `⚠ ${check.model_error}`;
        statusEl.style.color = 'var(--danger)';
      } else {
        statusEl.textContent = '✓ Ollama reachable';
    ...
    sel.innerHTML = '';
    if (!models.length) {
      sel.innerHTML = '<option value="">No models in Ollama — run: ollama pull llava</option>';
    ...
    const savedDefault = localStorage.getItem(OLLAMA_DEFAULT_MODEL_KEY);
    const defaultExists = savedDefault && models.some((m) => (m.name || m.model) === savedDefault);
    if (defaultExists) {
      ...
      const visionish = models.find((m) => {
        ...
        return (
          ...
          || n.includes('qwen2-vl')
          || n.includes('gemma3') && n.includes('it')
        );
      });
      if (visionish) sel.value = visionish.name || visionish.model;
    }
    sel.addEventListener('change', () => {
      updateOllamaModelCheck();
      updateOllamaDefaultBtnState();
    });
    if (defaultBtn) {
      ...
      defaultBtn.addEventListener('click', () => {
        ...
        if (val) {
          localStorage.setItem(OLLAMA_DEFAULT_MODEL_KEY, val);
          defaultBtn.classList.add('is-default');
          defaultBtn.title = `Default: ${val}`;
        }
      });
    }
    updateOllamaDefaultBtnState();
  } catch (e) {
    if (hostEl) hostEl.textContent = 'unreachable';
    if (statusEl) {
      statusEl.textContent = `⚠ ${escapeHtml(e.message || String(e))}`;
      statusEl.style.color = 'var(--danger)';
    }
    sel.innerHTML = `<option value="">${escapeHtml(e.message || String(e))}</option>`;
    if (defaultBtn) defaultBtn.hidden = true;
  }
}

function updateOllamaDefaultBtnState() {
  const sel = document.getElementById('ollamaModelSelect');
  const defaultBtn = document.getElementById('ollamaDefaultBtn');
  if (!sel || !defaultBtn) return;
  const val = sel.value?.trim();
  const saved = localStorage.getItem(OLLAMA_DEFAULT_MODEL_KEY);
  const isDefault = val && val === saved;
  defaultBtn.classList.toggle('is-default', isDefault);
  defaultBtn.title = isDefault ? `Default: ${val}` : 'Set as default model';
}
...
async function updateOllamaModelCheck() {
  const sel = document.getElementById('ollamaModelSelect');
  const statusEl = document.getElementById('ollamaCheckStatus');
  ...
  try {
    const check = await api(`/api/ai/ollama/check?model=${encodeURIComponent(model)}`);
    if (check.model_ok === false && check.model_error) {
      statusEl.textContent = `⚠ ${check.model_error}`;
      statusEl.style.color = 'var(--danger)';
    } else {
...
function renderAiIdentifyResult(data) {
  const wrap = document.getElementById('aiIdentifyOutput');
  if (!wrap) return;
  wrap.classList.remove('hidden');
  let html = '';
  if (data.shape_hint_error) {
    html += `<p class="muted">Silhouette hint: ${escapeHtml(data.shape_hint_error)}</p>`;
  } else if ((data.shape_hints || []).length) {
    html += `<p class="muted"><strong>Silhouette hints</strong> (lower distance ≈ closer to reference profile):</p><pre>${escapeHtml(JSON.stringify(data.shape_hints, null, 2))}</pre>`;
  }
  if (data.parsed && typeof data.parsed === 'object') {
    const p = data.parsed;
    const ranked = p.ranked_models || [];
    ranked.forEach((rm, i) => {
      const name = rm.name || '?';
      const master = state.masterKnives.find((m) => m.name && m.name.toLowerCase() === name.toLowerCase());
      const addLink = master ? `<a href="/?add=${master.id}" class="button-link add-to-inventory-btn">Add to inventory</a>` : '';
      html += `<div class="ai-parsed-card"><h4>${i + 1}. ${escapeHtml(name)}</h4>`;
      html += `<div class="muted">Confidence: ${escapeHtml(String(rm.confidence ?? ''))}</div>`;
      html += `<p>${escapeHtml(rm.rationale || '')}</p>`;
      if (addLink) html += `<div>${addLink}</div>`;
      html += '</div>';
    });
    if (p.caveats) {
      html += `<p class="muted"><strong>Caveats:</strong> ${escapeHtml(p.caveats)}</p>`;
    }
    if (p.shape_read) {
      html += `<p class="muted"><strong>Shape read:</strong> ${escapeHtml(p.shape_read)}</p>`;
    }
  }
  html += '<details class="ai-raw-details"><summary>Raw model response</summary><pre>';
  html += `${escapeHtml(data.raw_response || '')}</pre></details>`;
  wrap.innerHTML = html;
}
...
function initIdentifyPage() {
  document.querySelectorAll('.tab-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.identify-tab-panel').forEach((panel) => {
        const isKeyword = tab === 'keyword';
        const isPanel = (panel.id === 'keywordPanel' && isKeyword) || (panel.id === 'aiPanel' && !isKeyword);
        panel.classList.toggle('hidden', !isPanel);
        panel.setAttribute('aria-hidden', !isPanel);
      });
    });
  });
  ...
  loadOllamaUi().catch((err) => console.error(err));

  const aiPhotoInput = document.getElementById('aiPhotoInput');
  const aiPhotoPlaceholder = document.getElementById('aiPhotoPlaceholder');
  ...
  const aiPhotoRemoveBtn = document.getElementById('aiPhotoRemoveBtn');
  let aiPhotoObjectUrl = null;
  if (aiPhotoInput && aiPhotoPlaceholder && aiPhotoPreview && aiPhotoPreviewImg && aiPhotoRemoveBtn) {
    aiPhotoInput.addEventListener('change', () => {
      if (aiPhotoObjectUrl) {
        URL.revokeObjectURL(aiPhotoObjectUrl);
        aiPhotoObjectUrl = null;
      }
      const file = aiPhotoInput.files?.[0];
      if (file && file.type.startsWith('image/')) {
        aiPhotoObjectUrl = URL.createObjectURL(file);
        aiPhotoPreviewImg.src = aiPhotoObjectUrl;
        aiPhotoPlaceholder.classList.add('hidden');
        aiPhotoPreview.classList.remove('hidden');
      }
    });
    aiPhotoRemoveBtn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      aiPhotoInput.value = '';
      if (aiPhotoObjectUrl) {
      ...
      aiPhotoPreviewImg.removeAttribute('src');
      aiPhotoPlaceholder.classList.remove('hidden');
      aiPhotoPreview.classList.add('hidden');
    });
  }

  document.getElementById('aiIdentifyForm')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const form = e.target;
    const desc = (form.elements.description?.value || '').trim();
    const file = form.elements.image?.files?.[0];
    if (!desc && !file) {
      alert('Add a description and/or a photo.');
      return;
    }
    const modelVal = form.elements.model?.value?.trim();
    if (!modelVal) {
      alert('Select an Ollama model (check that Ollama is reachable).');
      return;
    }
    const fd = new FormData();
    fd.set('model', modelVal);
    fd.set('description', desc);
    if (file) fd.set('image', file);
    const cb = form.elements.include_shape_hint;
    fd.set('include_shape_hint', cb && cb.checked ? 'true' : 'false');
    const status = document.getElementById('aiIdentifyStatus');
    const btn = document.getElementById('aiSubmitBtn');
    status?.classList.remove('hidden');
    status.textContent = 'Calling Ollama… vision models can take 30–120s.';
